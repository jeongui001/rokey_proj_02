...
class TaskExecutor:
    ...
    def _compute_tool_track_tcp_offset(self, message):
        error = self._tcp_position_error(
            message, self.get_parameter('servo_pick.tool_track_frame_id').value)
        if error is None:
            return None
        offset = ToolTrack()
        offset.header = message.header
        offset.tool_class = message.tool_class
        offset.velocity = message.velocity
        offset.approaching = message.approaching
        offset.depth_valid = message.depth_valid
        offset.confidence = message.confidence
    ...
    def _on_tool_track_during_servo(self, msg):
        offset_msg = self._compute_tool_track_tcp_offset(msg)
        if offset_msg is None:
            # frame_id 불일치, NaN/Inf, TCP 조회 실패/신선도 미달 - 이번 프레임은
            # 유실된 것처럼 취급한다(ServoLoop.should_abort의 t_lost_s가 결국 감지한다).
            return
        self.servo_loop.on_tool_track(offset_msg)

    def _run_rt_tracking(
            self, goal_handle, *, name, message_type, topic, callback,
            servo, tick, validate_command, ready_parameter, period_parameter):
        ...
        try:
            ...
            if self.hardware_enabled and not rt_confirmed:
                ...
                while rclpy.ok():
                    ...
                    command = servo.step()
                    if (self.hardware_enabled and ready and rt_confirmed
